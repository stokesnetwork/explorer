...
const BlockDAGBox = () => {
  ...
  const initBox = async () => {
    ...
    let hashrateMax;
    try {
      hashrateMax = await getHashrateMax();
    } catch (e) {}
    ...
    if (dag_info && dag_info.virtualDaaScore !== undefined) {
      setVirtualDaaScore(dag_info.virtualDaaScore);
      localStorage.setItem("cacheVirtualDaaScore", dag_info.virtualDaaScore);
    }

    if (hashrate && hashrate.hashrate !== undefined) {
      setHashrate(hashrate.hashrate * 1000 * 1000 * 1000 * 1000);
      localStorage.setItem("cacheHashrate", hashrate.hashrate);
    }

    if (hashrateMax && hashrateMax.hashrate !== undefined) {
      setMaxHashrate(hashrateMax.hashrate * 1000 * 1000 * 1000 * 1000);
      localStorage.setItem("cacheHashrateMax", hashrateMax.hashrate);

      const ts = hashrateMax?.blockheader?.timestamp;
      if (ts !== undefined) {
        setMaxHashrateTimestamp(ts);
        localStorage.setItem("cacheMaxHashrateTimestamp", ts);
      }
    }

    const fr = feeEstimate?.normalBuckets?.[0]?.feerate;
    const pr = feeEstimate?.priorityBucket?.feerate;
    if (fr !== undefined) {
      setFeerate(fr);
    }
    if (pr !== undefined) {
      localStorage.setItem("feerate", pr);
    }
